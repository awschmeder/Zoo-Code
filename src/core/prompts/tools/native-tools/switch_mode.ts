...
When to use this vs new_task: switch_mode is the primary tool for adjusting capabilities. It changes the mode of the current task in place and preserves the existing conversation context. A common trigger for switch_mode is a FileRestrictionError -- when the current mode cannot edit a required file, switch to a mode whose allowed file patterns include it. Use new_task only when you need to delegate self-contained work to a subtask with fresh, isolated context.`
